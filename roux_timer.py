...
class roux_timer:
    ...
    def change_scheme(self):
        ...
        colour_canv.pack()
        csU = colour_canv.create_rectangle(
            50, 10, 20 + 60, 20 + 20, fill=self.colour_dict.get("Y")
        )
        colour_canv.tag_bind(
            csU, "<ButtonPress-1>", lambda _: self.change_particular_colour("U")
        )
        csL = colour_canv.create_rectangle(
            10, 50, 20 + 20, 20 + 60, fill=self.colour_dict.get("B")
        )
        colour_canv.tag_bind(
            csL, "<ButtonPress-1>", lambda _: self.change_particular_colour("L")
        )
        csF = colour_canv.create_rectangle(
            50, 50, 20 + 60, 20 + 60, fill=self.colour_dict.get("R")
        )
        colour_canv.tag_bind(
            csF, "<ButtonPress-1>", lambda _: self.change_particular_colour("F")
        )
        csR = colour_canv.create_rectangle(
            90, 50, 20 + 100, 20 + 60, fill=self.colour_dict.get("G")
        )
        colour_canv.tag_bind(
            csR, "<ButtonPress-1>", lambda _: self.change_particular_colour("R")
        )
        csB = colour_canv.create_rectangle(
            130, 50, 20 + 140, 20 + 60, fill=self.colour_dict.get("O")
        )
        colour_canv.tag_bind(
            csB, "<ButtonPress-1>", lambda _: self.change_particular_colour("B")
        )
        csD = colour_canv.create_rectangle(
            50, 90, 20 + 60, 20 + 100, fill=self.colour_dict.get("W")
        )
        colour_canv.tag_bind(
            csD, "<ButtonPress-1>", lambda _: self.change_particular_colour("D")
        )
    ...
    def print_cube(self):
        for x in range(3):
            for y in range(3):
                self.cube_space.create_rectangle(
                    10 * x + 50,
                    10 * y + 10,
                    10 * x + 60,
                    10 * y + 20,
